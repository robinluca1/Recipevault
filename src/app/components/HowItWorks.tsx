  ...
  return (
    <section className="py-24 px-6 bg-gradient-to-b from-yellow-50 to-amber-100/40">
      <div className="max-w-5xl mx-auto">
        ...
        >
          <span className="inline-block px-4 py-1 bg-amber-200 text-amber-700 rounded-full mb-4" style={{ fontSize: "0.8125rem", fontWeight: 600 }}>
            HOW IT WORKS
          </span>
          ...
          {steps.map((s, i) => (
            ...
            >
              <div className="relative w-28 h-28 rounded-3xl bg-white border-2 border-amber-200 shadow-md flex flex-col items-center justify-center mb-6 z-10">
                ...
                <span className="absolute -top-3 -right-3 w-7 h-7 bg-amber-400 rounded-full flex items-center justify-center text-amber-900" style={{ fontSize: "0.7rem", fontWeight: 800 }}>
                  {s.n}
                </span>
              </div>
              <h3 className="text-amber-950 mb-2" style={{ fontSize: "1rem", fontWeight: 700 }}>{s.title}</h3>
              <p className="text-amber-700/60 max-w-xs" style={{ fontSize: "0.9rem", lineHeight: 1.65 }}>{s.desc}</p>
            </motion.div>
          ))}
        </div>
      </div>
